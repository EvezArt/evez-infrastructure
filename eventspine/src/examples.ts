...
import { EventSpine, OperationalEvent } from "./index";
...
// ---------------------------------------------------------------------------
// State Rehydration — Crash Recovery
// ---------------------------------------------------------------------------
...
async function rehydrationExample() {
  const spine = new EventSpine({ dataDir: "/tmp/spine-data" });
  await spine.open();
  ...
  const state = await spine.rehydrate<MarketState>(initialState, (s, event) => {
    switch (event.kind) {
      case "SIGNAL_UPDATE":
        return { ...s, currentModel: (event.payload.model as string) ?? s.currentModel };
      case "USAGE_RECORDED":
        return {
          ...s,
          totalCost: s.totalCost + ((event.payload.costUsd as number) ?? 0),
